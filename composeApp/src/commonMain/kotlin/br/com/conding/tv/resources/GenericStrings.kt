package br.com.conding.tv.resources

internal object GenericStrings {
    const val PROJECT_NAME = "Spring Sale"
    const val CREATE_ONE_ACCOUNT = "Criar uma conta"
    const val CREATE_MY_ACCOUNT = "Criar minha conta"
    const val RECOVER_MY_ACCOUNT = "Recuperar acesso"
    const val EMAIL = "Email"
    const val PASSWORD = "Senha"
    const val OR = "ou"
    const val ENTER_YOUR_ACCOUNT = "Acessar minha conta"
    const val FORGOT_PASS = "Esqueceu sua senha?"
    const val NOT_BLANK_OR_EMPTY = "Preencha todos os campos!"
    const val VERSION = "Versão:"
    const val YOUR_ACTION = "Você deseja?"
    const val EMPTY_TEXT = ""
    const val WARNING = "Atenção!"
    const val FILTER = "Filtro"
    const val CHANGE_TO_OTHER_ACCOUNT = "Trocar de conta"
    const val EXIT = "Sair do Programa"
    const val CATEGORIES = "Categorias"
    const val PRODUCTS = "Produtos"
    const val SETTINGS = "Configurações"
    const val ID = "Código"
    const val CATEGORY_NAME = "Nome da categoria"
    const val NEW_NAME_CATEGORY = "Novo nome da categoria"
    const val SAVE_CATEGORY = "Salvar categoria"
    const val UPDATE_CATEGORY = "Atualizar categoria"
    const val DELETE_CATEGORY = "Apagar categoria"
    const val ADD_CATEGORIES = "Adicionar categorias"
    const val NO_CATEGORIES_SELECTED = "Nenhuma categoria selecionada"
    const val CANCEL = "Cancelar"
    const val CONFIRM = "Confirmar"
    const val SEARCH = "Pesquisar"
    const val SIZE_LIST = "Tamanho da lista"
    const val LOADING = "Carregando…"
    const val HOME = "Home"
    const val ALERT = "Alerta!"
    const val ACTUAL_NAME = "Nome atual"
    const val NAME = "Nome"
    const val SURNAME = "Sobrenome"
    const val USERNAME = "Nome de Usuário"
    const val NUMBER = "Número"
    const val ADJUSTMENTS = "Ajustes"
    const val LIST_PRODUCTS = "Lista de Produtos"
    const val DETAILS_PRODUCT = "Detalhes do Produto"
    const val UPDATE_PRODUCT = "Atualizar Produto:"
    const val UPDATE_PRODUCT_MOBILE = "Atualizar Produto"
    const val UPDATE_PRICE_PRODUCT = "Atualizar preço do produto"
    const val UPDATE_PRICE_PRODUCT_MOBILE = "Atualizar preço"
    const val RESTOCK_PRODUCT = "Repor Estoque do Produto"
    const val RESTOCK_PRODUCT_MOBILE = "Repor estoque"
    const val CREATE_PRODUCT = "Criar Novo Produto"
    const val NEW_NAME_PRODUCT = "Novo nome do Produto"
    const val DELETE_PRODUCT = "Apagar Produto"
    const val EMPTY_LIST_PRODUCTS = "Nenhum Produto Encontrado!"
    const val EMPTY_LIST_CATEGORIES = "Nenhuma Categoria Encontrada!"
    const val MESSAGE_ZERO_DOUBLE = "O preço precisa ser maior que 0!"
    const val CONFIRM_UPDATE = "Confirmar Atualização"
    const val QUANTITY = "Quantidade"
    const val PRICE = "Preço"
    const val OPTIONS = "Opções"
    const val RESOURCE_UNAVAILABLE = "Recurso indisponível!"
    const val PAST_CODE = "colar o código enviado"
    const val VERIFY_CODE_SENT = "Verificar o código enviado"
    const val CONFIRM_PASSWORD = "Confirmar Senha"
    const val INVALID_PASSWORD = "Senhas não coincidem!"
    const val PASSWORD_ERROR_SIZE = "Sua senha tem menos de 8 digitos!"
    const val CREATE_NEW_PASSWORD = "Criar nova senha"
    const val YOUR_MAIN_EMAIL = "Digite seu email principal"
    const val INVALID_EMAIL = "Este email é inválido!"
    const val REGISTERED_EMAIL = "Digite o email registrado"
    const val SEND_CODE_VERIFICATION = "Enviar código de verificação"
    const val SEND_RECOVER_TOKEN = "Enviar Código de recuperação"
    const val SEND_CODE_VERIFICATION_MOBILE = "Enviar código"
    const val RESULT = "Resultado:"
    const val SAVE_MODIFIER = "Salvar modificação"
    const val ITEM_TO_ADD = "Criar novo item:"
    const val ITEM_TO_DELETE = "Item selecionado para excluir:"
    const val ITEM_TO_UPDATE = "Item selecionado para atualizar:"
    const val ITEM_TO_UPDATE_PRICE = "Item selecionado para atualizar o preço:"
    const val ITEM_TO_RESTOCK = "Item selecionado para atualizar o estoque:"
    const val DATA_USER = "Dados do Usuário"
    const val SAVE_DATA_USER = "Salvar dados do usuário"
    const val ALTER_DATA_USER = "Alterar dados do usuário"
    const val DELETE_MY_ACCOUNT = "Apagar minha conta"
    const val YES = "Sim"
    const val NO = "Não"
    const val WARNING_THE_DELETE_MY_ACCOUNT = "Você realmente deseja apagar sua conta?"
}
